...
std::vector<std::string> EvenOrOdd (std::vector<std::string> v) {
  ...
  for (std::string str : v) {
    ...
    if (sum % 2 == 0) even.push_back(word);
    else odd.push_back(word);
    sum = 0;
  }

  // Return the correct string
  if (even.size() < odd.size()) return odd;
  else if (odd.size() < even.size()) return even;
  else return empty;
}
...
int main () {
  ...
  if (!EvenOrOdd(v).empty())
    for (std::string str : EvenOrOdd(v))
      std::cout << str << '\n';
