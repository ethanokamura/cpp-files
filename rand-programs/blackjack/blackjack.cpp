...
using namespace paperwrld;

BlackjackHand::BlackjackHand(const BlackjackHand &that) {
  for (std::string card : that._hand) this->_hand.insert(card);
}
...
BlackjackHand::BlackjackHand(std::initializer_list<std::string> cards) {
  ...
  for (std::string card : cards) *this += card;
}
...
std::string BlackjackHand::to_string() const {
  std::string temp;
  for (std::string card : this->_hand) temp += card + ' ';
  temp = temp.substr(0, temp.length() - 1);
  return '[' + temp + ']';
};

std::size_t BlackjackHand::size() const { return this->_hand.size(); }
...
auto BlackjackHand::operator<=>(const BlackjackHand &that) const {
  if (this->numeric_value() == 21 && that.numeric_value() == 21 &&
      this->size() != that.size())
...
bool BlackjackHand::operator!=(const BlackjackHand &that) const {
  if (this->numeric_value() == 21 && that.numeric_value() &&
      this->size() != that.size())
    return true;
  return this->numeric_value() != that.numeric_value() ? true : false;
}
